  ...
  description: INodeTypeDescription = {
    ...
    properties: [
      ...
      // ----------------------------------------------------------------
      {
        ...
        options: [
          ...
          },
          {
            ...
            description: 'Get many contact lists (segments) including member counts',
          },
          {
            name: 'Remove Member',
            value: 'removeMember',
      ...
      // ----------------------------------------------------------------
      {
        ...
        options: [
          ...
          },
          {
            ...
            description: 'Assign the conversation to a team member (empty = unassign)',
          },
          {
            name: 'Delete',
            value: 'delete',
      ...
      },
      {
        ...
        description: 'Name of the n8n binary property that holds the file to upload',
      },
      {
        ...
        options: [
          ...
          { displayName: 'Folder ID', name: 'folder_id', type: 'string', default: '' },
        ],
      },
      {
      ...
      // ----------------------------------------------------------------
      {
        ...
        options: [
          ...
          },
          {
            ...
            description: 'Send an HTML email in an email conversation (with subject)',
          },
        ],
        default: 'send',
      },
      ...
      // ----------------------------------------------------------------
      {
        ...
        description: 'At least one of name/email/phone/external ID should be set',
        options: [
          {
          ...
          },
          {
            ...
            type: 'string',
            default: '',
            description: 'Your own contact ID (for dedup / matching with your CRM)',
          },
        ...
        options: [
          {
            ...
            type: 'json',
            default: '{}',
            description: 'Custom attributes as a JSON object. A value of `null` removes the attribute.',
          },
          {
      ...
      // ----------------------------------------------------------------
      {
        ...
        default: '',
        displayOptions: {
          show: { resource: ['contactList'], operation: ['addMember', 'removeMember'] },
        },
      },
      {
        displayName: 'Contact ID',
        ...
        options: [
          {
            ...
            placeholder: 'name@example.com',
            default: '',
            description: 'Exact email lookup (via blind index, without decrypting all rows)',
          },
        ],
      },
      ...
      // ----------------------------------------------------------------
      {
        ...
        displayOptions: {
          show: {
            ...
            operation: ['get', 'setStatus', 'assign', 'addTag', 'removeTag', 'delete'],
          },
        },
      },
      {
        ...
        required: true,
        default: '',
        displayOptions: { show: { resource: ['message'], operation: ['send', 'sendEmail'] } },
      },
      ...
      },
      {
        ...
        default: 50,
        displayOptions: {
          show: { resource: ['conversation'], operation: ['getAll'], returnAll: [false] },
        },
        description: 'Max number of results to return',
      },
      {
        ...
        options: [
          {
            ...
            type: 'string',
            default: '',
            description: 'Only conversations of this contact (get many conversations of a contact)',
          },
          {
      ...
      // ----------------------------------------------------------------
      {
        ...
        default: 'open',
        displayOptions: { show: { resource: ['conversation'], operation: ['setStatus'] } },
      },
      ...
      // ----------------------------------------------------------------
      {
        ...
        description: 'User ID of the team member. Leave empty to remove the assignment.',
      },
      ...
      // ----------------------------------------------------------------
      {
        ...
        default: '',
        displayOptions: {
          show: { resource: ['conversation'], operation: ['addTag', 'removeTag'] },
        },
      ...
      try {
        ...
        if (resource === 'contact') {
          if (operation === 'create') {
            ...
            for (const key of ['name', 'email', 'phone', 'external_id'] as const) {
              const value = fields[key];
              if (typeof value === 'string' && value.trim() !== '') body[key] = value.trim();
            }
            if (fields.attributes !== undefined && fields.attributes !== '') {
              body.attributes = parseJsonParam.call(this, fields.attributes, 'Attributes', i);
            }
            ...
            const body: IDataObject = {};
            for (const key of ['name', 'email', 'phone', 'consent_status'] as const) {
              const value = fields[key];
              if (typeof value === 'string' && value.trim() !== '') body[key] = value.trim();
            }
            if (fields.attributes !== undefined && fields.attributes !== '') {
          ...
          if (operation === 'getAll') {
            ...
            const memberContactId = this.getNodeParameter('memberContactId', i) as string;
            responseData = await ticketZeroApiRequest.call(
              this,
              'POST',
          ...
          const conversationId =
            operation === 'getAll' ? '' : (this.getNodeParameter('conversationId', i) as string);
          const convPath = `/v1/api-public/conversations/${encodeURIComponent(conversationId)}`;

          if (operation === 'get') {
            responseData = await ticketZeroApiRequest.call(this, 'GET', convPath);
          } else if (operation === 'getAll') {
            const returnAll = this.getNodeParameter('returnAll', i) as boolean;
            const filters = this.getNodeParameter('conversationFilters', i, {}) as IDataObject;
            const qs: IDataObject = {};
            if (filters.status) qs.status = filters.status;
            if (typeof filters.inbox_id === 'string' && filters.inbox_id.trim() !== '') {
              qs.inbox_id = filters.inbox_id.trim();
            }
            if (typeof filters.contact_id === 'string' && filters.contact_id.trim() !== '') {
            ...
            responseData = await ticketZeroApiRequest.call(this, 'PATCH', convPath, { status });
          } else if (operation === 'assign') {
            const assigneeUserId = (this.getNodeParameter('assigneeUserId', i) as string).trim();
            responseData = await ticketZeroApiRequest.call(this, 'PATCH', convPath, {
            ...
            responseData = await ticketZeroApiRequest.call(this, 'DELETE', convPath);
          }
        } else if (resource === 'note') {
          const noteConversationId = this.getNodeParameter('noteConversationId', i) as string;
          const notesBase = `/v1/api-public/conversations/${encodeURIComponent(
            noteConversationId,
          )}/notes`;
          if (operation === 'add') {
            const noteBody = this.getNodeParameter('noteBody', i) as string;
            responseData = await ticketZeroApiRequest.call(this, 'POST', notesBase, {
              body: noteBody,
            });
          } else if (operation === 'getAll') {
            const resp = await ticketZeroApiRequest.call(this, 'GET', notesBase);
            responseData = (resp?.data as IDataObject[]) ?? [];
          } else if (operation === 'delete') {
          ...
          if (operation === 'get') {
            responseData = await ticketZeroApiRequest.call(this, 'GET', '/v1/api-public/me');
          }
          ...
          if (operation === 'send') {
            ...
            const reqBody: IDataObject = { body: emailBody, content_type: 'email_html' };
          ...
          if (operation === 'getAll') {
            const folderId = (this.getNodeParameter('fileFolderId', i, '') as string).trim();
            const qs: IDataObject = {};
            ...
            const fields = this.getNodeParameter('fileUploadFields', i, {}) as IDataObject;
            ...
            if (typeof fields.folder_id === 'string' && fields.folder_id.trim() !== '') {
